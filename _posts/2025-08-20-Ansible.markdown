...
- [本讲内容](#本讲内容)
  - [ansible参考资料: https://ansible-tran.readthedocs.io/en/latest/docs/intro.html](#ansible参考资料-httpsansible-tranreadthedocsioenlatestdocsintrohtml)
  - [配置 Ansible](#配置-ansible)
    - [控制机 (M3 MacOS)](#控制机-m3-macos)
    - [免密登录](#免密登录)
  - [ansible 模式](#ansible-模式)
    - [ad-hoc](#ad-hoc)
    - [playbook](#playbook)
...
[ansible参考资料][ansible参考资料]

[ansible参考资料]: https://ansible-tran.readthedocs.io/en/latest/docs/intro.html
---
...
注意使用command模块, 不得出现shell变量$name, 也不得出现特殊符号如 > < | ; & 这些. 如果需要用这些特殊符号, 要使用shell模块.
...
<figure style="text-align: center;">
<img src="/assets/img/ansible17.png" alt="" width="500">
<figcaption>获取所有被管理机器的负载信息</figcaption>
</figure>

让客户端机器先切换目录, 然后打印当前工作目录. 在 Ansible 的 command 模块里，chdir 是从目标机器的 根目录 / 开始算路径的，而不是从你的家目录 ~ 开始。

<figure style="text-align: center;">
<img src="/assets/img/ansible18.png" alt="" width="500">
<figcaption>chdir</figcaption>
</figure>

使用creates参数判断该文件是否存在, 存在则跳过, 不存在则执行. 如判断/Desktop是否存在, 当前存在, 所以跳过pwd操作.

<figure style="text-align: center;">
<img src="/assets/img/ansible19.png" alt="" width="500">
<figcaption>creates</figcaption>
</figure>

使用removes参数, 存在则执行, 不存在则跳过: 如判断/Desktop是否存在, 当前存在, 所以执行pwd操作.

<figure style="text-align: center;">
<img src="/assets/img/ansible20.png" alt="" width="500">
<figcaption>removes</figcaption>
</figure>

如果查找一个不存在的目录, 且不使用creates或removes, 就会报错. 使用的话会直接跳过报错. 

warn参数决定是否提供告警信息:

比如执行 ansible macs -m command -a "chmod 000 /etc/hosts" 修改权限, 默认会有红色警告. 如果warn参数置false就可以取消告警. 这里就不演示了.

ansible macs -m command -a "chmod 000 /etc/hosts warn=False" 
...
<figure style="text-align: center;">
<img src="/assets/img/ansible21.png" alt="" width="500">
<figcaption>批量查询进程信息</figcaption>
</figure>

批量写入文件:

ansible macs -m shell -a "echo 你知道什么是江湖吗 > /test.txt"

批量执行脚本(该脚本必须在客户端机器上存在, 否则会报错文件不存在. 这是shell的特点. 还有一个专门执行脚本的script模块.)步骤: 创建文件夹/.sh脚本文件 >>  写入脚本内容 >> 赋予脚本可执行权限 >> 执行脚本/处理warning. 所有操作都在控制主机上完成.

```
ansible macs -m shell -a "mkdir -p /test/myscripts/; echo 'hostname' > /test/myscripts/hostname.sh; chmod +x /test/myscripts/hostname.sh; bash /test/myscripts/hostname.sh  warn=False"
# 可以cat 检查 
...
把控制主机上的脚本远程传输到被控机上执行. 比起shell, script更强大, 在控制机上有一份脚本, 就可以在被控机上运行, 而不需要传送后才能执行.

在管理节点上创建脚本 >> 授权 >> 执行.

ansible macs -m script -a "脚本路径"
