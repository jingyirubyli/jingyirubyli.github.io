...
困扰我好几天的问题: 此时访问http://localhost/index.php只显示文字<?php phpinfo(); ?>, 而不是php默认页面. 刚刚仔细看了一下安装好php后的介绍如下:
...
# 这一行大家都知道
LoadModule php_module /opt/homebrew/opt/php/lib/httpd/modules/libphp.so
# 下面几行容易忽略, 因为网上的教程都没有提到
<FilesMatch \.php$>
    SetHandler application/x-httpd-php
</FilesMatch>
```

修改好httpd.conf后重启apache即可:

<figure style="text-align: center;">
<img src="/assets/img/apache6.png" alt="" width="400">
<figcaption>成功访问phpinfo</figcaption>
</figure>

此时访问localhost/index.html也可以正常显示 it works!

如果不想要直接访问localhost时自动显示某个页面, 修改DirectoryIndex中内容, 不设置链接先后顺序即可:

<figure style="text-align: center;">
<img src="/assets/img/apache8.png" alt="" width="400">
<figcaption>显示目录</figcaption>
</figure>
