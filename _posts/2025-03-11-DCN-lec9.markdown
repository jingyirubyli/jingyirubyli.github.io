...
寄存器数组宽度register array width（和条目数 # of entries）在编译时就被定义. 那么如何处理大于寄存器宽度的值？解决方法是分多个阶段将值跨越多个寄存器

<figure style="text-align: center;">
<img src="/assets/img/l9p16.png" alt="The register array width (and # of entries) is defined at compile time" width="200"><img src="/assets/img/l9p17.png" alt="Span the value across multiple registers in multiple stages" width="400">
...
1. 关键概念
	-	Bitmap（位图）：
    -	指示该键的值存储在哪些寄存器数组中。
    -	bitmap = 111 表示数据分布在 3 个寄存器数组（Register Array 0, 1, 2）。
	-	Index（索引）：
